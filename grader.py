...
def grade_ex1(pm: StagedPassManager, backend: IBMBackend):
    grade_pm(pm, backend, 'ex1', pass_score=800, circuit=cost_layer)

def grade_ex2(pm: StagedPassManager, backend: IBMBackend):
    grade_pm(pm, backend, 'ex2', pass_score=750, circuit=cost_layer)

def grade_ex3(pm: StagedPassManager, backend: IBMBackend):
    grade_pm(pm, backend, 'ex3', pass_score=700, circuit=cost_layer)

def grade_ex4(pm: TranspilerService, backend: IBMBackend):
    grade_pm(pm, backend, 'ex4', pass_score=700, circuit=cost_layer)

def grade_ex5(pm: StagedPassManager, backend: IBMBackend):
    grade_pm(pm, backend, 'ex5', pass_score=400, circuit=cost_layer)

def grade_ex6(pm: StagedPassManager, backend: IBMBackend):
    grade_pm(pm, backend, 'ex6', pass_score=170, circuit=cost_layer)
...
def grade_pm(pm: Union[StagedPassManager, TranspilerService], backend: IBMBackend, ex_id: str, pass_score: int, circuit: QuantumCircuit):
    ...
    if average_score > pass_score:
        print(Fore.RED + f"Fail. Average score: {average_score} > {pass_score}.")
    else:
        print(Fore.GREEN + f"Pass. Average score: {average_score} <= {pass_score}.")
